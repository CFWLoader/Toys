...
input_data = read.csv('./dataset/aqipm25.csv', fileEncoding = "UTF-8-BOM")
...
png(file = './dataset/aqi.png')

write.table(toJSON(as.numeric(input_data$AQI)), './dataset/aqi_dist.json', row.names = FALSE, col.names = FALSE, quote = FALSE)

hist(as.numeric(as.numeric(input_data$AQI)), main = 'AQI Dist', xlab = 'AQI Values')

dev.off()

png(file = './dataset/pm25.png')

write.table(toJSON(as.numeric(input_data$PM2.5)), './dataset/pm25_dist.json', row.names = FALSE, col.names = FALSE, quote = FALSE)
